...
```
En una localidad del interior del país hay dos bancos A y B. El 22% de los habitantes tiene cuenta en A, el 37% en B y el 47% no tiene cuenta. a) ¿Cuál es el porcentaje de habitantes que tiene cuenta en ambos bancos?; b) de los que tienen cuenta en A, ¿qué porcentaje tiene cuenta en B?; c) de los que tienen cuenta corriente, ¿qué porcentaje tiene cuenta en B?
```

### Tema II — Ejercicio 7  (follow_up)

**Motivo:** Detecté que el enunciado tiene varias partes (a, b, c, …) y cada una pide algo distinto. Pegá la consigna de UNA parte por vez (ej: sólo el inciso a) para que pueda resolverla.
**Resp. esperada:** a) 62%; b) 40,32%; c) 0,2383; d) 37%.
...
### Tema II — Ejercicio 26  (follow_up)

**Motivo:** No pude identificar el modelo estadístico. ¿Cuál es? (Ej: Binomial, Poisson, etc.)
**Resp. esperada:** 1/2 = 0,5.
...
```
El control de calidad para cierto tipo de motor incluye dos pruebas: A (ensayo de sobrecarga) y B (ensayo de consumo). El 5% falla en la prueba A, el 6% en la prueba B y el 90% en ninguna. a) Indique si las fallas en las pruebas son sucesos estadísticamente independientes, justificando numéricamente la respuesta; b) de los que no fallan en A, ¿qué porcentaje falla en B?
```
...
**Motivo:** No pude identificar el modelo estadístico. ¿Cuál es? (Ej: Binomial, Poisson, etc.)
**Resp. esperada:** P(0) = 0,7109 ; P(1) = 0,2547 ; P(2) = 0,0328 ; P(3) = 0,0016.
...
```
Se debe satisfacer un pedido de 8 discos de freno para un prototipo experimental, siendo 0,25 la probabilidad de que uno de ellos esté fuera de especificación. El costo de fabricación de cada disco es de U$s 250 pero, si en la partida inicial no se obtienen los 8 discos buenos, se deberán hacer los adicionales, uno por uno, a un costo de U$s 400 cada uno, manteniéndose la probabilidad de defectuoso. El comprador paga además U$s 150 por cada disco bueno sobrante. a) Si se decide fabricar inicialmente 11 discos, ¿cuál es el costo esperado del pedido?; b) ¿Cómo se puede optimizar dicho costo?
```

### Tema III — Ejercicio 32  (follow_up)

**Motivo:** Detecté que el enunciado tiene varias partes (a, b, c, …) y cada una pide algo distinto. Pegá la consigna de UNA parte por vez (ej: sólo el inciso a) para que pueda resolverla.
**Resp. esperada:** a) U$s 2.164,24; b) hay que repetir el cálculo para distintos valores de n, el valor óptimo se obtiene con n = 16    U$s 2.110,64. En este punto se puede usar el SOLVER del Excel.

```
Con el fin de obtener 12 piezas mecanizadas sin deformaciones, se funden 18 piezas debido a que en el proceso de fundición puede salir alguna pieza con deformaciones. El costo de fundir una pieza (incluido el material) es de 40 U$s y el costo de mecanizarla es de 120 U$s si se parte de una pieza sin deformaciones y de 400 U$s si se parte de una pieza con deformaciones. La experiencia muestra que el 15% de las piezas fundidas presenta deformaciones. a) Calcular el costo esperado por lote si sólo se piensa en mecanizar 12 piezas y el resto se desecha; b) ¿Cómo se puede optimizar dicho costo?
```

### Tema IV — Ejercicio 5  (follow_up)

**Motivo:** No pude identificar el modelo estadístico. ¿Cuál es? (Ej: Binomial, Poisson, etc.)
**Resp. esperada:** a) 0,5695 = 56,95%; b) 0,5469.
...
**Motivo:** Detecté que el enunciado tiene varias partes (a, b, c, …) y cada una pide algo distinto. Pegá la consigna de UNA parte por vez (ej: sólo el inciso a) para que pueda resolverla.
**Resp. esperada:** a) 0,2383; b) 0,2775; c) 0,2172.
...
```
En un banco a las cuentas pertenecientes a actividades comerciales se les cobra un costo administrativo. Para presupuestar los gastos administrativos del próximo trimestre se desea saber el costo administrativo esperado de estas cuentas. Para ello se estableció como “cuenta tipo” aquella cuyo monto de crédito se encuentre entre U$s 4.500 ± 500. El costo administrativo básico de toda cuenta es de 20 U$s. Las cuentas cuyo saldo sea menor a 4.000 U$s tiene un costo administrativo adicional fijo de 10 U$s y hay que adicionar un valor de 5 U$s más por cada 1.000 U$s de diferencia con respecto a 4.0
```
...
```
El control de expedición de una tela que se expende en rollos de 400 metros, se efectúa revisando todos los rollos separando aquellos en los que se encuentren al menos 2 fallas; la inspección de cada rollo se detiene al encontrar la 2ª falla. Se sabe que el proceso productivo trabaja con una tasa de 1 falla cada 1.000 metros en promedio. Calcular: a) El número medio de fallas en los rollos separados (o sea, aquellos que no salen a expedición) y en los no separados (o sea, aquellos que salen a expedición); b) el costo medio de muestreo por rollo, sabiendo que la inspección cuesta 0,10 U$s por m
```
